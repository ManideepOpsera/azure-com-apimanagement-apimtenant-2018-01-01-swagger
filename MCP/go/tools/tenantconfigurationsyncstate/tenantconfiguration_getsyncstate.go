...
func Tenantconfiguration_getsyncstateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/tenant/%s/syncState%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, configurationName, queryString)
		...
		var result models.TenantConfigurationSyncStateContract
...
func CreateTenantconfiguration_getsyncstateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_tenant_configurationName_syncState",
		mcp.WithDescription("Gets the status of the most recent synchronization between the configuration database and the Git repository."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tenantconfiguration_getsyncstateHandler(cfg),
	}
}
