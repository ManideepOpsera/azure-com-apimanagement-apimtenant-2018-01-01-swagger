...
func Tenantconfiguration_deployHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeployConfigurationParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/tenant/%s/deploy%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, configurationName, queryString)
...
func CreateTenantconfiguration_deployTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_tenant_configurationName_deploy",
		mcp.WithDescription("This operation applies changes from the specified Git branch to the configuration database. This is a long running operation and could take several minutes to complete."),
		...
		mcp.WithBoolean("force", mcp.Description("Input parameter: The value enforcing deleting subscriptions to products that are deleted in this update.")),
		mcp.WithString("branch", mcp.Required(), mcp.Description("Input parameter: The name of the Git branch from which the configuration is to be deployed to the configuration database.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Tenantconfiguration_deployHandler(cfg),
	}
}
