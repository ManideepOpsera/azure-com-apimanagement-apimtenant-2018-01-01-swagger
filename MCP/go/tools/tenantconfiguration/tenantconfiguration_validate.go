...
func Tenantconfiguration_validateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/tenant/%s/validate%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, configurationName, queryString)
...
func CreateTenantconfiguration_validateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_tenant_configurationName_validate",
		mcp.WithDescription("This operation validates the changes in the specified Git branch. This is a long running operation and could take several minutes to complete."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Tenantconfiguration_validateHandler(cfg),
	}
}
